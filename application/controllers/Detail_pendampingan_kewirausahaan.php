...
class Detail_pendampingan_kewirausahaan extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('detailPendampinganKewirausahaanModel','pendampingan');
	}
	...
	public function data($id='')
	{
		...
		$this->skin->view('detail_pendampingan_kewirausahaan/index',$data);
	}

	public function get_all($id)
	{
		$get=$this->pendampingan->getData($id);
		$data = [];
		// echo "<pre>";
		// print_r ($get);
		// echo "</pre>";exit();
		foreach ($get as $key => $value) {
			...
			$a = [
				...
				// // '<a type="button" href="'.base_url('detail_pendampingan_kewirausahaan/edit/').$id.'/'.$value->id.'" class="btn btn-success"><i class="fas fa-edit"></i></a>'.
	...
	public function add($id)
	{
		...
		$data['data'] = $this->pendampingan->getDataKab($id);
		$data['data']['kuota_terpakai'] = $this->pendampingan->getKuota($id);
		$data['kec'] = $this->pendampingan->getKec($data['data']['kode_kab_kota']);

		$this->skin->view('detail_pendampingan_kewirausahaan/add',$data);
	}
	...
	public function create()
	{
		...
		$check = $this->pendampingan->cekDesaKel($kd_desa);
	...
	public function edit($id_target,$id='')
	{
		...
		$data['datakab'] = $this->pendampingan->getDataKab($id_target);
		$data['datakab']['kuota_terpakai'] = $this->pendampingan->getKuota($id_target);

		$data['data'] = $this->global->get_by_one('wa_targetkk_desa',$id,'id');
		$data['kec'] = $this->pendampingan->getKec($data['datakab']['kode_kab_kota']);
		$data['data']['nama_kecamatan'] = $this->pendampingan->getNamaKec($data['data']['kode_desa_kelurahan'])['nama_kecamatan'];
		$data['data']['kode_kecamatan'] = $this->pendampingan->getNamaKec($data['data']['kode_desa_kelurahan'])['kode_kecamatan'];
		$this->skin->view('detail_pendampingan_kewirausahaan/edit',$data);
	...
	{
		$data=$this->pendampingan->getKel($kode);
...
/* End of file Detail_pendampingan_kewirausahaan.php */
/* Location: ./application/controllers/Detail_pendampingan_kewirausahaan.php */