...
class Detail_penyuluhan extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('detailPenyuluhanModel','penyuluhan');
	}
	...
	public function data($id='')
	{
		...
		$this->skin->view('detail_penyuluhan/index',$data);
	}

	public function get_all($id)
	{
		$get=$this->penyuluhan->getData($id);
		$data = [];
		// echo "<pre>";
		// print_r ($get);
		// echo "</pre>";exit();
		foreach ($get as $key => $value) {
			...
			$a = [
				...
				// '<a type="button" href="'.base_url('detail_penyuluhan/edit/').$id.'/'.$value->id.'" class="btn btn-success"><i class="fas fa-edit"></i></a>'.
				'<a type="button"  style="display:inline" href="'.base_url('penyuluhan/upload/').$value->id.'" class="btn btn-success"><i class="fas fa-edit" ></i></a>'
	...
	public function add($id)
	{
		...
		$data['data'] = $this->penyuluhan->getDataKab($id);
		$data['data']['kuota_terpakai'] = $this->penyuluhan->getKuota($id);
		$data['kec'] = $this->penyuluhan->getKec($data['data']['kode_kab_kota']);

		$this->skin->view('detail_penyuluhan/add',$data);
	}
	...
	public function create()
	{
		...
		$check = $this->penyuluhan->cekDesaKel($kd_desa);
	...
	public function edit($id_target,$id='')
	{
		...
		$data['datakab'] = $this->penyuluhan->getDataKab($id_target);
		$data['datakab']['kuota_terpakai'] = $this->penyuluhan->getKuota($id_target);

		$data['data'] = $this->global->get_by_one('wa_targetkk_desa',$id,'id');
		$data['kec'] = $this->penyuluhan->getKec($data['datakab']['kode_kab_kota']);
		$data['data']['nama_kecamatan'] = $this->penyuluhan->getNamaKec($data['data']['kode_desa_kelurahan'])['nama_kecamatan'];
		$data['data']['kode_kecamatan'] = $this->penyuluhan->getNamaKec($data['data']['kode_desa_kelurahan'])['kode_kecamatan'];
		$this->skin->view('detail_penyuluhan/edit',$data);
	...
	{
		$data=$this->penyuluhan->getKel($kode);
