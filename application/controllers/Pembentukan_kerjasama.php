...
class Pembentukan_kerjasama extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('PembentukanKerjasamaModel','pembentukan');
	}

	public function index()
	{
		$data['title'] = 'Home - Entry Subject Object - Tahun Pertama - Pembentukan Kerjasama';
		$this->skin->view('pembentukan_kerjasama/index',$data);
	}
	...
	public function get_all()
	{
		...
		foreach ($get as $key => $value) {
			$a = [
				...
				rupiah($value->anggaran_kerjasama),rupiah($value->realisasi_kerjasama),
				'<div style="display:inline-flex">
				<a type="button" style="display:inline" href="'.base_url('detail_pembentukan_kerjasama/data/').$value->id.'" class="btn btn-success"><i class="fas fa-search"></i></a>'.
				'<a type="button"  style="display:inline" href="'.base_url('pembentukan_kerjasama/upload/').$value->id.'" class="btn btn-primary"><i class="fas fa-upload" ></i></a>'.
	...
	public function add()
	{
		...
		$data['prov']=$this->pembentukan->getProv();
		$data['title']= 'Home - Entry Subject Object - Tahun Pertama - Pembentukan Kerjasama - Baru';
		$this->skin->view('pembentukan_kerjasama/add',$data);
	}
	...
	public function create()
	{
		...
		$checkWil = $this->pembentukan->cekWIlTahun($kd_kabkot,$thn_anggaran);
	...
	public function edit($id='')
	{
		...
		// $data['prov']=$this->pembentukan->getProv();
		$data['prov']=$this->pembentukan->getProv();
		$data['title']= 'Home - Entry Subject Object - Tahun Pertama - Pembentukan Kerjasama - Edit';
		$data['data'] = $this->global->get_by_one('wa_targetkk',$id,'id');
		$this->skin->view('pembentukan_kerjasama/edit',$data);
	...
	{
		$data=$this->pembentukan->getKab($kode);
	...
	public function upload_sk1($id)
        {
        	...
        	if ($_FILES['file_foto_sk1']) {
                ...
                $config['upload_path']          = './uploads/pembentukan_kerjasama/';
	...
	{
		$data['id']=$id;
		$data['title']= 'Home - Entry Subject Object - Tahun Pertama - Pembentukan Kerjasama - Upload';
		$data['data'] = $this->global->get_by_one('dt_pengembangan_rencana_usaha',$id,'id');
		$this->skin->view('pembentukan_kerjasama/upload',$data);

	}
	public function get_upload($id)
	{
		$get = $this->global->get_by_result('fl_pembentukan_kerjasama',$id,'targetkk_id');
		...
		foreach ($get as $key => $value) {
			...
			$a = [
				$key+1,$jenis_evidence,tanggal_indonesia($value->tanggal_pembentukan_kerjasama),
	 ...
	 public function do_upload()
        {
        	...
        	$data = [
        			...
        			'tanggal_pembentukan_kerjasama' => $this->input->post('tanggal_pembentukan_kerjasama',true),
    			];
			if($id){
        		$this->db->where('id', $id);
				$ins = $this->db->update('fl_pembentukan_kerjasama', $data);
			
            }else{

				$this->db->insert('fl_pembentukan_kerjasama', $data);
				 $id = $this->db->insert_id();
            }
            ...
            if ($_FILES['files']['name']) {
                ...
                else
                {
                	...
                	$data = [
	                    			...
	                    			'dir_name' => 'uploads/pembentukan_kerjasama/'.$this->upload->data()['file_name'],
	                			];
	                	$this->db->where('id', $id);
    					$this->db->update('fl_pembentukan_kerjasama', $data);
	...
	{
		$data = $this->global->get_by_one('fl_pembentukan_kerjasama',$id,'id');
		echo json_encode($data);
		...
		public function delete_upload($id='')
	{
		...
		$this->db->where('id', $id);
		$del = $this->db->delete('fl_pembentukan_kerjasama');
		if($del)
		{
			echo json_encode(['sts' => 'success']);
		}else{
	...
	{
		$data=$this->pembentukan->getRealisasi($kab,$tahun);
		echo json_encode($data);
	}
	...
	{
		$data =[
					'realisasi_kerjasama' => str_replace('.', '', $this->input->post('realisasi',true))
