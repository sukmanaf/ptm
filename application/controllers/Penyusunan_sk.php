...
class Penyusunan_sk extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('penyusunanSkModel','penyusunan');
	}

	public function index()
	{
		$data['title'] = 'Home - Entry Subject Object - Tahun Kedua - Penyusunan SK Pembentukan Kelompok Masyarakat';
		$this->skin->view('penyusunan_sk/index',$data);
	}
	...
	public function get_all()
	{
		...
		foreach ($get as $key => $value) {
			$a = [
				...
				rupiah($value->anggaran_penyusunan_sk),rupiah($value->realisasi_penyusunan_sk),
				'<div style="display:inline-flex">
				<a type="button" style="display:inline" href="'.base_url('detail_penyusunan_sk/data/').$value->id.'" class="btn btn-success"><i class="fas fa-search"></i></a>'.
				'<a type="button"  style="display:inline" href="'.base_url('penyusunan_sk/upload/').$value->id.'" class="btn btn-primary"><i class="fas fa-upload" ></i></a>'.
	...
	public function add()
	{
		...
		$data['title']= 'Home - Entry Subject Object - Tahun Kedua - Penyusunan SK Pembentukan Kelompok Masyarakat - Baru';
		$this->skin->view('penyusunan_sk/add',$data);
	}
	...
	public function edit($id='')
	{
		...
		// $data['prov']=$this->penyusunan->getProv();
		$data['prov']=$this->penyusunan->getProv();
		$data['title']= 'Home - Entry Subject Object - Tahun Kedua - Penyusunan SK Pembentukan Kelompok Masyarakat - Edit';
		$data['data'] = $this->global->get_by_one('wa_targetkk',$id,'id');
		$this->skin->view('penyusunan_sk/edit',$data);
	...
	public function upload_sk1($id)
        {
        	...
        	if ($_FILES['file_foto_sk1']) {
                ...
                $config['upload_path']          = './uploads/penyusunan_sk/';
	...
	{
		$data['id']=$id;
		$data['title']= 'Home - Entry Subject Object - Tahun Kedua - Penyusunan SK Pembentukan Kelompok Masyarakat - Upload';
		$data['data'] = $this->global->get_by_one('dt_pengembangan_rencana_usaha',$id,'id');
		$this->skin->view('penyusunan_sk/upload',$data);

	}
	public function get_upload($id)
	{
		$get = $this->global->get_by_result('fl_penyusunan_sk',$id,'targetkk_id');
		...
		foreach ($get as $key => $value) {
			...
			$a = [
				$key+1,$jenis_evidence,tanggal_indonesia($value->tanggal_penyusunan_sk),
	 ...
	 public function do_upload()
        {
        	...
        	$data = [
        			...
        			'tanggal_penyusunan_sk' => $this->input->post('tanggal_penyusunan_sk',true),
    			];
			if($id){
        		$this->db->where('id', $id);
				$ins = $this->db->update('fl_penyusunan_sk', $data);
			
            }else{

				$this->db->insert('fl_penyusunan_sk', $data);
				 $id = $this->db->insert_id();
            }
            ...
            if ($_FILES['files']['name']) {
                ...
                else
                {
                	...
                	$data = [
	                    			...
	                    			'dir_name' => 'uploads/penyusunan_sk/'.$this->upload->data()['file_name'],
	                			];
	                	$this->db->where('id', $id);
    					$this->db->update('fl_penyusunan_sk', $data);
	...
	{
		$data = $this->global->get_by_one('fl_penyusunan_sk',$id,'id');
		echo json_encode($data);
		...
		public function delete_upload($id='')
	{
		...
		$this->db->where('id', $id);
		$del = $this->db->delete('fl_penyusunan_sk');
		if($del)
		{
			echo json_encode(['sts' => 'success']);
		}else{
	...
	{
		$data =[
					'realisasi_penyusunan_sk' => str_replace('.', '', $this->input->post('realisasi',true))
