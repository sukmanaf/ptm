...
class Penguatan_kelembagaan extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('PenguatanKelembagaanModel','penguatan');
	}

	public function index()
	{
		$data['title'] = 'Home - Entry Subject Object - Tahun Kedua - Penguatan Kelembagaan';
		$this->skin->view('penguatan_kelembagaan/index',$data);
	}
	...
	public function get_all()
	{
		...
		foreach ($get as $key => $value) {
			$a = [
				$key+1,@$value->nama_provinsi,@$value->nama_kab_kota,@$value->tahun,@$value->target_kk,rupiah($value->anggaran_kelembagaan),rupiah($value->realisasi_kelembagaan),
				'<div style="display:inline-flex"><a type="button" style="display:inline" href="'.base_url('detail_penguatan_kelembagaan/data/').$value->id.'" class="btn btn-success"><i class="fas fa-search"></i></a>'.
				'<a type="button"  style="display:inline" href="'.base_url('penguatan_kelembagaan/upload/').$value->id.'" class="btn btn-primary"><i class="fas fa-upload" ></i></a>'.
				'<button type="button" id="realisasi" onclick="realisasi(\''.$value->kode_kab_kota.'\','.$value->tahun_anggaran.',\''.$value->nama_kab_kota.'\')" class="btn btn-warning "><i class="fas fa-edit"></i></button></div>'
			];
			array_push($data,$a);
		}
	...
	public function add()
	{
		...
		$data['prov']=$this->penguatan->getProv();
		$data['title']= 'Home - Entry Subject Object - Tahun Kedua - Penguatan Kelembagaan - Baru';
		$this->skin->view('penguatan_kelembagaan/add',$data);
	}
	...
	public function create()
	{
		...
		$checkWil = $this->penguatan->cekWIlTahun($kd_kabkot,$thn_anggaran);
	...
	public function edit($id='')
	{
		...
		// $data['prov']=$this->penguatan->getProv();
		$data['prov']=$this->penguatan->getProv();
		$data['title']= 'Home - Entry Subject Object - Tahun Kedua - Penguatan Kelembagaan - Edit';
		$data['data'] = $this->global->get_by_one('wa_targetkk',$id,'id');
		$this->skin->view('penguatan_kelembagaan/edit',$data);
	...
	{
		$data=$this->penguatan->getKab($kode);
	...
	public function upload_sk1($id)
        {
        	...
        	if ($_FILES['file_foto_sk1']) {
                ...
                $config['upload_path']          = './uploads/penguatan_kelembagaan/';
	...
	{
		$data['id']=$id;
		$data['title']= 'Home - Entry Subject Object - Tahun Kedua - Penguatan Kelembagaan - Upload';
		$data['data'] = $this->global->get_by_one('dt_pengembangan_rencana_usaha',$id,'id');
		$this->skin->view('penguatan_kelembagaan/upload',$data);

	}
	public function get_upload($id)
	{
		$get = $this->global->get_by_result('fl_penguatan_kelembagaan',$id,'targetkk_id');
		...
		foreach ($get as $key => $value) {
			...
			$a = [
				$key+1,$jenis_evidence,tanggal_indonesia($value->tanggal_penguatan_kelembagaan),
	 ...
	 public function do_upload()
        {
			...
			if($id){
        		$this->db->where('id', $id);
				$ins = $this->db->update('fl_penguatan_kelembagaan', $data);
			
            }else{

				$this->db->insert('fl_penguatan_kelembagaan', $data);
				 $id = $this->db->insert_id();
            }
            ...
            if ($_FILES['files']['name']) {
                ...
                else
                {
                	...
                	$data = [
                    			...
                    			'dir_name' => 'uploads/penguatan_kelembagaan/'.$this->upload->data()['file_name'],
    							'tanggal_penguatan_kelembagaan' => $this->input->post('tanggal_penguatan_kelembagaan',true),
								'created_by'	=> $sess['username'],
	                		];
	                	$this->db->where('id', $id);
    					$this->db->update('fl_penguatan_kelembagaan', $data);
	...
	{
		$data = $this->global->get_by_one('fl_penguatan_kelembagaan',$id,'id');
		echo json_encode($data);
		...
		public function delete_upload($id='')
	{
		...
		$this->db->where('id', $id);
		$del = $this->db->delete('fl_penguatan_kelembagaan');
		if($del)
		{
			echo json_encode(['sts' => 'success']);
		}else{
	...
	{
		$data=$this->penguatan->getRealisasi($kab,$tahun);
		echo json_encode($data);
	}
	...
	{
		$data =[
					'realisasi_kelembagaan' => str_replace('.', '', $this->input->post('realisasi_kelembagaan',true))
