	  ...
	  <div class="card card-outline card-primary">
	    <div class="card-header">
	      ...
	      <div class="card-tools">
	      	      <!-- <a  style="float: right" href="<?=base_url('penguatan_kelembagaan/add')?>" class="btn btn-warning pull-right">
      ...
      <div class="modal-body">
 		<form action="#" id="postForm" class="form-horizontal" enctype="multipart/form-data" method="post">
        	...
        	<div class="row">
        		...
        		<div class="col-md-12">
					<div class="form-group">
						<label for='realisasi_kelembagaan'>Realisasi</label>
						<input type="text" name="realisasi_kelembagaan" required onkeyup ="realisasiChange($(this).val())" id="realisasi_kelembagaan" class="form-control">
					</div>
        		</div>
        	</div>	
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("penguatan_kelembagaan/edit_realisasi") ?>'; //get form action url
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url();?>penguatan_kelembagaan/get_all',
		        data: function ( d ) {
    ...
    function realisasi(kode_kab_kota,tahun_anggaran) {
    	...
    	$.get( "<?php echo base_url("penguatan_kelembagaan/getRealisasi/") ?>"+kode_kab_kota+'/'+tahun_anggaran, function( data ) {
			data = JSON.parse(data)
			console.log(data)
			var realisasi = data.realisasi_kelembagaan == null ? 0 : data.realisasi_kelembagaan;
	    	$('#tahun_anggaran').val(data.tahun_anggaran)
	    	$('#kode_kab_kota').val(data.kode_kab_kota)
	    	$('#nama_kab_kota').val(data.nama_kab_kota)
	    	$('#anggaran').val(rupiah(data.anggaran_kelembagaan))
	    	$('#realisasi_kelembagaan').val(rupiah(realisasi))
    ...
    function realisasiChange(val) {
        ...
        if (parseInt(val) >= anggaran) {
        	$('#realisasi_kelembagaan').val(rupiah(anggaran))
        }else{
        	$('#realisasi_kelembagaan').val(rupiah(val))
	    }
