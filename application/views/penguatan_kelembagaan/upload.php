							...
							<div class="col-6">
								<div class="form-group">
									...
									<div class="input-group date"  data-target-input="nearest">
										...
										<input type="date" id="tanggal_penguatan_kelembagaan" class="clear form-control" name="tanggal_penguatan_kelembagaan" value="<?=date('Y-m-d')?>">
									</div>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="row">
									<div class="col">
										<div class="form-group">
											<a href="<?=base_url('penguatan_kelembagaan')?>" class="form-control btn btn-secondary"><label >Tutup</label></a>
										</div>
									</div>
									<div class="col">
										<div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("penguatan_kelembagaan/do_upload") ?>'; //get form action url
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url('penguatan_kelembagaan/get_upload/').$id;?>',
		        data: function ( d ) {
    ...
    function editS(id) {
    	...
    	// // console.log(tab[id]);
    	$.get( "<?php echo base_url("penguatan_kelembagaan/get_edit_upload/") ?>"+id, function( data ) {
			data = JSON.parse(data)
			console.log(data)
    		$('#jenis_evidence').val(data.jenis_evidence).trigger("change");
    		$('#tanggal_penguatan_kelembagaan').val(data.tanggal_penguatan_kelembagaan);
    		$('#id').val(id)
		});
    ...
    function dels(id) {
	    $.get( "<?php echo base_url("penguatan_kelembagaan/delete_upload/") ?>"+id, function( response ) {
			response = JSON.parse(response)
        	if (response.sts == 'success') {
