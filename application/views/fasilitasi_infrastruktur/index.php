    ...
    <div class="container-fluid">
	  <div class="card card-outline card-primary">
	    <div class="card-header">
	    	<a style="" href="<?=base_url()?>/subjectobject/tahun_ketiga" class="btn btn-default">
	        <i class="fas fa-arrow-left mr-2 text-white"></i>Kembali </a>
	      <!-- <a  style="float: right" href="<?=base_url('fasilitasi_infrastruktur/add')?>" class="btn btn-warning pull-right">
	    ...
	    <div style="padding: 20px">
	      	...
	      	<!-- <a class="btn btn-primary btn-sm" target="_blank" href="<?php echo base_url("export_pdf/fasilitasi_infrastruktur_sarana/") ?>">Export Sarana</a> -->
	      	<!-- <a class="btn btn-primary btn-sm" target="_blank" href="<?php echo base_url("export_pdf/fasilitasi_infrastruktur_prasarana/") ?>">Export Prasaarana</a> -->
	    
	    </div>
	    <div class="card-body table-responsive">
	      <div id="table-front_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
	      	<!-- <button onclick="pdf()" class="btn btn-warning btn-sm">Export PDF</button> -->
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("fasilitasi_infrastruktur/edit_realisasi") ?>'; //get form action url
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url();?>fasilitasi_infrastruktur/get_all',
		        data: function ( d ) {
    ...
    function del(id) {
    	$.get( "<?=base_url('fasilitasi_infrastruktur/delete/')?>"+id, function( response ) {
		  	response = JSON.parse(response)
        	if (response.sts == 'success') {
    ...
    function realisasi(kode_kab_kota,tahun_anggaran) {
    	...
    	$.get( "<?php echo base_url("fasilitasi_infrastruktur/getRealisasi/") ?>"+kode_kab_kota+'/'+tahun_anggaran, function( data ) {
			...
			if (data==null) {
		    	...
		    	$('#anggaran').val(rupiah(data.anggaran_insfratruktur_pendukung))
		    	$('#idrealisasi').val(rupiah(data.realisasi_insfratruktur_pendukung))
