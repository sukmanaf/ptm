							...
							<div class="form-group">
								...
								<select id="instansi_id" name="instansi_id" required class="form-control select2" style="width: 100%;">
									...
									<?php foreach ($key as $key => $value) : ?>
										<?php if ($value->id == $data['instansi_id']) {
											$sel = 'selected';
										} else {
											$sel = '';
										} ?>
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input name="nama_instansi" id="nama_instansi" type="text" value="<?= $data['nama_instansi']; ?>" class="form-control clear">
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input name="contac_person" id="contac_person" type="text" value="<?= $data['contac_person']; ?>" class="form-control clear">
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input name="alamat_instansi" id="alamat_instansi" type="text" value="<?= $data['alamat_instansi']; ?>" class="form-control clear">
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input name="email" id="email" type="text" value="<?= $data['email']; ?>" class="form-control clear">
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input name="nomor_telp" id="nomor_telp" type="text" value="<?= $data['nomor_telp']; ?>" class="form-control clear">
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#postForm").submit(function(event) {
			...
			var post_url = '<?php echo base_url("detail_instansi_swasta/update") ?>'; //get form action url
			...
			$.ajax({
				...
				if (response.sts == 'success') {
					...
					$('.clear').val("")
					window.location.replace('<?= base_url("detail_instansi_swasta") ?>');
	...
	function nikChange(nik) {
		event.preventDefault(); //prevent default action 
		$.get("<?php echo base_url("detail_instansi_swasta/detailNik/") ?>" + nik, function(data) {
			data = JSON.parse(data)
			console.log(data.luas)
			$('#keanggotaan').val(data.keanggotaan)
			$('#detail_instansi_swasta').val(data.detail_instansi_swasta)
