					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input name="jenis_pendampingan" id="jenis_pendampingan" type="text" value="<?= $data['jenis_pendampingan'] ?>" class="form-control clear">
							</div>
						</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#postForm").submit(function(event) {
			...
			var post_url = '<?php echo base_url("jenis_pendampingan/update") ?>'; //get form action url
			...
			$.ajax({
				...
				if (response.sts == 'success') {
					...
					$('.clear').val("")
					window.location.replace('<?= base_url("jenis_pendampingan") ?>');
