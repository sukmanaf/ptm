			...
			<div class="card-body">
				<form action="#" id="postForm" class="form-horizontal" enctype="multipart/form-data" method="post">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								<label for="kode_sub_prestasi_kinerja">Kode Sub Sektor Usaha</label>
								<input name="kode_prestasi_kinerja" maxlength="4" id="kode_prestasi_kinerja" type="text" value="<?= $data['kode_subsektor_usaha'] ?>" class="form-control clear">
							</div>
						</div>
					</div>
					<div class="row">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#postForm").submit(function(event) {
			...
			var post_url = '<?php echo base_url("prestasi_kinerja/update") ?>'; //get form action url
			...
			$.ajax({
				...
				if (response.sts == 'success') {
					...
					$('.clear').val("")
					window.location.replace('<?= base_url("prestasi_kinerja") ?>');
	...
	function nikChange(nik) {
		event.preventDefault(); //prevent default action 
		$.get("<?php echo base_url("prestasi_kinerja/detailNik/") ?>" + nik, function(data) {
			data = JSON.parse(data)
			console.log(data.luas)
			$('#keanggotaan').val(data.keanggotaan)
			$('#prestasi_kinerja').val(data.prestasi_kinerja)
