<section class="content-header">
      		...
      		<div class="card card-outline card-primary">
				<div class="card-body">
 					<form action="#" id="postForm" class="form-horizontal" enctype="multipart/form-data" method="post">
                		...
                		<input type="hidden" name="kode_desa_kelurahan" value="<?=@$kode_desa_kelurahan?>">
                		<input type="hidden" name="id" id="id" class="clear" value="">
                		<input type="hidden" name="id_targetkk" id="id_targetkk" class="" value="<?=@$id_targetkk?>">
                		<input type="hidden" name="id_targetkk_desa" id="id_targetkk_desa" class="" value="<?=@$id?>">
                		<input type="hidden" name="batas" id="batas" class="" value="<?=@$penlok['jumlah']?>">
 ...
 -->									<select name="akses_pemasaran" id="akses_pemasaran" class="form-control">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="row">
									<div class="col">
										<div class="form-group">
											<a href="<?=base_url('detail_pengembangan/data/').$id_targetkk?>" class="form-control btn btn-secondary"><label >Tutup</label></a>
										</div>
									</div>
									<div class="col">
										<div class="form-group">
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("detail_pengembangan/update") ?>'; //get form action url
    ...
    function nikChange(nik) {
	    // event.preventDefault(); //prevent default action 
	    if(nik != ''){
			$.get( "<?php echo base_url("detail_pengembangan/detailNik/") ?>"+nik, function( data ) {
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url('detail_pengembangan/get_detail/').$id;?>',
		        data: function ( d ) {
    ...
    function editS(id) {
    	...
    	// // console.log(tab[id]);
    	$.get( "<?php echo base_url("detail_pengembangan/get_edit/") ?>"+id, function( data ) {
			data = JSON.parse(data)
			// console.log(data)
    		$('#nik').val(data.nik).trigger("change");
    		$('#kapasitas').val(data.kapasitas);
    		$('#akses_pemasaran').val(data.akses_pemasaran);
    		$('#kebutuhan_akses').val(data.kebutuhan_akses).trigger("change");
    ...
    function dels(id) {
	    $.get( "<?php echo base_url("detail_pengembangan/delete/") ?>"+id, function( response ) {
			response = JSON.parse(response)
        	if (response.sts == 'success') {
