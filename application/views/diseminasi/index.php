    ...
    <div class="container-fluid">
	  <div class="card card-outline card-primary">
	    ...
	    <div style="padding: 20px">
	      	<!-- <a class="btn btn-primary btn-sm" target="_blank" href="<?php echo base_url("export_pdf/diseminasi/") ?>">Export Laporan</a> -->
	    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("diseminasi/edit_realisasi") ?>'; //get form action url
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url();?>diseminasi/get_all',
		        data: function ( d ) {
    ...
    function del(id) {
    	$.get( "<?=base_url('diseminasi/delete/')?>"+id, function( response ) {
		  	response = JSON.parse(response)
        	if (response.sts == 'success') {
    ...
    function realisasi(kode_kab_kota,tahun_anggaran) {
    	...
    	$.get( "<?php echo base_url("diseminasi/getRealisasi/") ?>"+kode_kab_kota+'/'+tahun_anggaran, function( data ) {
			...
			if (data==null) {
		    	...
		    	$('#anggaran').val(rupiah(data.anggaran_diseminasi))
		    	$('#idrealisasi').val(rupiah(data.realisasi_diseminasi))
