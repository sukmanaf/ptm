	...
	<div class="container-fluid">
		<div class="card card-outline card-primary">
			<!-- <div class="card-header">
				<a style="float: right" href="<?= base_url('klasifikasi_sektor_usaha/add') ?>" class="btn btn-warning pull-right">
					...
					<table id="table-front" class="table table-hover dataTable no-footer dtr-inline" aria-describedby="table-front_info" style="width: 100%;">
						<thead>
							<tr>
								<th>No</th>
								<th> Sektor Usaha</th>
								<th> Sub Sektor Usaha</th>
								<th> Jenis Sub Sektor Usaha</th>
							</tr>
						</thead>
					</table>
				</div>
...
<script type="text/javascript">
	...
	function loadDt() {
		$('#table-front').DataTable({
			ajax: {
				url: '<?php echo base_url(); ?>klasifikasi_sektor_usaha/get_all',
				data: function(d) {
	...
	function dels(id) {
		$.get("<?= base_url('klasifikasi_sektor_usaha/delete/') ?>" + id, function(response) {
