								...
								<div class="row">
									<div class="col">
										<div class="form-group">
											...
											<a class="btn btn-success form-control" target="_blank" href="<?php echo base_url('lintor_bkpm/download_template'); ?>">Download Template Excel</a>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#postForm").submit(function(event) {
			...
			var post_url = '<?php echo base_url("lintor_bkpm/create") ?>'; //get form action url
			...
			$.ajax({
				...
				if (response.sts == 'success') {
					...
					$('.clear').val("")
					window.location.replace('<?= base_url("lintor_bkpm") ?>');
	...
	function loadDt() {
		$('#table-front').DataTable({
			ajax: {
				url: '<?php echo base_url(); ?>lintor_bkpm/get_all',
				data: function(d) {
	...
	function dels(id) {
		$.get("<?= base_url('lintor_bkpm/delete/') ?>" + id, function(response) {
			response = JSON.parse(response)
			if (response.sts == 'success') {
