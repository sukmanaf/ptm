    ...
    <div class="container-fluid">
	  <div class="card card-outline card-primary">
	    <div class="card-header">
	      ...
	      <!-- <a  style="float: right" href="<?=base_url('fasilitasi_akses/add')?>" class="btn btn-warning pull-right">
	    ...
	    <div style="padding: 20px">
	      	<!-- <a class="btn btn-primary btn-sm" target="_blank" href="<?php echo base_url("export_pdf/fasilitasi_akses/") ?>">Export Laporan</a> -->
	    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("fasilitasi_akses/edit_realisasi") ?>'; //get form action url
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url();?>fasilitasi_akses/get_all',
		        data: function ( d ) {
    ...
    function del(id) {
    	$.get( "<?=base_url('fasilitasi_akses/delete/')?>"+id, function( response ) {
		  	response = JSON.parse(response)
        	if (response.sts == 'success') {
    ...
    function realisasi(kode_kab_kota,tahun_anggaran) {
    	...
    	$.get( "<?php echo base_url("fasilitasi_akses/getRealisasi/") ?>"+kode_kab_kota+'/'+tahun_anggaran, function( data ) {
			...
			if (data==null) {
		    	...
		    	$('#anggaran').val(rupiah(data.anggaran_akses_pemasaran))
		    	$('#idrealisasi').val(rupiah(data.realisasi_akses_pemasaran))
