	  ...
	  <div class="card card-outline card-primary">
	    <div class="card-header">
	      ...
	      <div class="card-tools">
	      	      <!-- <a  style="float: right" href="<?=base_url('penyusunan_data/add')?>" class="btn btn-warning pull-right">
        	...
        	<div class="row">
        		...
        		<div class="col-md-12">
					<div class="form-group">
						...
						<input type="text" name="realisasi_penyusunan_data" required onkeyup ="realisasiChange($(this).val())" id="realisasi_penyusunan_data" class="numbers form-control">
					</div>
        		</div>
        	</div>	
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("penyusunan_data/edit_realisasi") ?>'; //get form action url
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url();?>penyusunan_data/get_all',
		        data: function ( d ) {
    ...
    function dels(id) {
    	$.get( "<?=base_url('penyusunan_data/delete/')?>"+id, function( response ) {
		  	response = JSON.parse(response)
        	if (response.sts == 'success') {
 ...
 function realisasi(kode_kab_kota,tahun_anggaran) {
    	...
    	$.get( "<?php echo base_url("penyusunan_data/getRealisasi/") ?>"+kode_kab_kota+'/'+tahun_anggaran, function( data ) {
			data = JSON.parse(data)
			console.log(data)
			var real = data.realisasi_penyusunan_data == null ? 0 : data.realisasi_penyusunan_data;
			$('#tahun_anggaran').val(data.tahun_anggaran)
	    	$('#kode_kab_kota').val(data.kode_kab_kota)
	    	$('#nama_kab_kota').val(data.nama_kab_kota)
	    	$('#anggaran').val(rupiah(data.anggaran_penyusunan_data))
	    	$('#realisasi_penyusunan_data').val(rupiah(real))
    ...
    function realisasiChange(val) {
        ...
        if (parseInt(val) >= anggaran) {
        	$('#realisasi_penyusunan_data').val(rupiah(anggaran))
        }else{
        	$('#realisasi_penyusunan_data').val(rupiah(val))
	    }
