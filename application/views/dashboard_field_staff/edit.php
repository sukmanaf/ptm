			...
			<div class="card-body">
				<form action="#" id="postForm" class="form-horizontal" enctype="multipart/form-data" method="post">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								<label for="kode_sub_dashboard_field_staff">Kode Sub Sektor Usaha</label>
								<input name="kode_dashboard_field_staff" maxlength="4" id="kode_dashboard_field_staff" type="text" value="<?= $data['kode_subsektor_usaha'] ?>" class="form-control clear">
							</div>
						</div>
					</div>
					<div class="row">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="row">
								<div class="col">
									<div class="form-group">
										<a href="<?= base_url('dashboard_field_staff') ?>" class="form-control btn btn-secondary"><label>Tutup</label></a>
									</div>
								</div>
								<div class="col">
									<div class="form-group">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#postForm").submit(function(event) {
			var kode = $("#kode_dashboard_field_staff").val()
			...
			var post_url = '<?php echo base_url("dashboard_field_staff/update") ?>'; //get form action url
			...
			$.ajax({
				...
				if (response.sts == 'success') {
					...
					$('.clear').val("")
					window.location.replace('<?= base_url("dashboard_field_staff") ?>');
	...
	function tampil_data(table, colum, id) {
		var url = ''
		var tadata = ''
		urls = "<?php echo base_url(); ?>dashboard_field_staff/tampildata/" + table + "/" + colum + "/" + id;
		$.ajax({
			url: urls,
	...
	function nikChange(nik) {
		event.preventDefault(); //prevent default action 
		$.get("<?php echo base_url("dashboard_field_staff/detailNik/") ?>" + nik, function(data) {
			data = JSON.parse(data)
			console.log(data.luas)
			$('#keanggotaan').val(data.keanggotaan)
			$('#dashboard_field_staff').val(data.dashboard_field_staff)
