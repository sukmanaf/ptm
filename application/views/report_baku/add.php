					...
					<div class="row">
						<div class="col-md-6">
							<div class="row">
								<div class="col">
									<div class="form-group">
										<a href="<?= base_url('report_baku') ?>" class="form-control btn btn-secondary"><label>Tutup</label></a>
									</div>
								</div>
								<div class="col">
									<div class="form-group">
...
<script type="text/javascript">
	...
	function combo(divname, table = '', colum = '1', id = '1', set = '') {
		url3 = "<?php echo base_url(); ?>report_baku/tampildata/" + table + "/" + colum + "/" + id + "/combobox";
	...
	function tampil_data(table, colum, id) {
		var url = ''
		var tadata = ''
		urls = "<?php echo base_url(); ?>report_baku/tampildata/" + table + "/" + colum + "/" + id;
		$.ajax({
			url: urls,
	...
	$(document).ready(function() {
		...
		$("#postForm").submit(function(event) {
			...
			var post_url = '<?php echo base_url("report_baku/create") ?>'; //get form action url
			...
			$.ajax({
				...
				if (response.sts == 'success') {
					...
					$('.clear').val("")
					window.location.replace('<?= base_url("report_baku") ?>');
