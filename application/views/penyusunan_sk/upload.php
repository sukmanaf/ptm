						...
						<div class="row">
							<div class="col-6">
								<div class="form-group">
									<label for='tanggal_sk'>Tanggal Penyusunan SK Pembentukan Kelompok Masyarakat</label>
									<div class="input-group date"  data-target-input="nearest">
										...
										<input type="date" id="tanggal_penyusunan_sk" class="clear form-control" name="tanggal_penyusunan_sk" value="<?=date('Y-m-d')?>">
									</div>
						...
						<div class="row">
							<div class="col-md-6">
								<div class="row">
									<div class="col">
										<div class="form-group">
											<a href="<?=base_url('penyusunan_sk')?>" class="form-control btn btn-secondary"><label >Tutup</label></a>
										</div>
									</div>
									<div class="col">
										<div class="form-group">
			        					...
			        					<table id="table-front" class="table table-hover dataTable no-footer dtr-inline" aria-describedby="table-sarana_info" style="width: 100%;">
											<thead class="">
												<tr>
													...
													<th>Tanggal Penyusunan SK Pembentukan Kelompok Masyarakat</th>
													<th>Aksi</th>
												</tr>
											</thead>
											<tbody id="tableBody">
...
<script type="text/javascript">
    $(document).ready(function(){
		...
		$("#postForm").submit(function(event){
	        ...
	        var post_url = '<?php echo base_url("penyusunan_sk/do_upload") ?>'; //get form action url
    ...
    function loadDt() {
    	$('#table-front').DataTable({
		    ajax: {
		        url: '<?php echo base_url('penyusunan_sk/get_upload/').$id;?>',
		        data: function ( d ) {
    ...
    function editS(id) {
    	...
    	// // console.log(tab[id]);
    	$.get( "<?php echo base_url("penyusunan_sk/get_edit_upload/") ?>"+id, function( data ) {
			data = JSON.parse(data)
			console.log(data)
    		$('#jenis_evidence').val(data.jenis_evidence).trigger("change");
    		$('#tanggal_penyusunan_sk').val(data.tanggal_penyusunan_sk);
    		$('#id').val(id)
		});
    ...
    function dels(id) {
	    $.get( "<?php echo base_url("penyusunan_sk/delete_upload/") ?>"+id, function( response ) {
			response = JSON.parse(response)
        	if (response.sts == 'success') {
