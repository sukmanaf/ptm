...
class DetailPenguatanKelembagaanModel extends CI_Model {
	...
	{
		$this->db->select('wa_kelompok_usaha.*,wa_kuesioner_re.id_targetkk_desa,wa_kuesioner_re.nama_responden_utama,ms_kelompok_usaha.nama_kelompok');
		$this->db->join('wa_kuesioner_re', 'wa_kuesioner_re.nik = wa_kelompok_usaha.nik', 'left');
		$this->db->join('ms_kelompok_usaha', 'ms_kelompok_usaha.id = wa_kelompok_usaha.id_kelompok_usaha', 'left');
		$this->db->where('wa_kuesioner_re.id_targetkk_desa', $id);
		return $this->db->get('wa_kelompok_usaha')->result();
	}
	...
	public function cekNik($nik='')
	{
		...
		return $this->db->get('wa_kelompok_usaha')->num_rows();
	}

	public function getKelompokUsaha($id_targetkk_desa)
	{
		$this->db->select('ms_kelompok_usaha.*');
		$this->db->where('ms_kelompok_usaha.status', 'Aktif');
		$this->db->where('wa_targetkk_desa.id', $id_targetkk_desa);
		$this->db->join('wa_targetkk_desa', 'substr(wa_targetkk_desa.kode_desa_kelurahan::text,1,4) = ms_kelompok_usaha.kabupaten', 'left');
		return $this->db->get('ms_kelompok_usaha')->result();
	}
}
